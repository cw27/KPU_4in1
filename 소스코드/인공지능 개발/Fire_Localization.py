...
# numpy를 이용하면 배열과 행렬의 덧셈, 뺄셈 곱셈, 나눗셈 등 연산을 사용할 수 있다.

class DuplicateRemover:
    def init(self, dirname, hash_size=8):
        self.dirname = dirname
        self.hash_size = hash_size # 폴더 이름과 hash 크기를 초기화한다.
    ...
    def find_duplicates(self):
        ...
        fnames = os.listdir(self.dirname) # 지정한 디렉토리 내 모든 파일과 디렉토리 리스트(list)를 리턴한다.
        hashes = {}
        duplicates = []
        print("Finding Duplicates Now!\n")
        for image in fnames: # fnames 리스트 안에 있는 이미지를 반복문을 이용하여 관리한다.
            with Image.open(os.path.join(self.dirname, image)) as img: 
                temp_hash = imagehash.average_hash(img, self.hash_size) # hash_size를 회색 이미지로 변화, 축소하고 평균값을 계산하고
                # 왼쪽에서 오른쪽으로 픽셀을 하나씩 검사한다.
                if temp_hash in hashes:
                    print("Duplicate {} \nfound for Image {}!\n".format(image, hashes[temp_hash]))
                    duplicates.append(image) #
                else:
                    hashes[temp_hash] = image # 중복이 아니면 hashes[temp_hash]에 이미지를 넣는다.
...
if (a.strip().lower() == "y"):
                for duplicate in duplicates:
                    ...
                    # 파일의 크기(size)를 받아온다.
                    os.remove(os.path.join(self.dirname, duplicate)) # os 라이브러리를 사용하여 중복 제거한다.
...
dirname = "./images/1" # 현재 파일에 있는 images 안에 있는 파일 /1을 참조한다.

# Remove Duplicates 중복 제거!
dr = DuplicateRemover(dirname) # dr 변수 안에 dirname을 초기화한다.
dr.find_duplicates() # 중복을 찾고 제거한다.
지은 — 어제 오후 2:13
https://github.com/cw27/KPU_4in1
GitHub
GitHub - cw27/KPU_4in1
Contribute to cw27/KPU_4in1 development by creating an account on GitHub.
GitHub - cw27/KPU_4in1
박보성 — 어제 오후 3:22
''' import glob # glob 모듈의 glob 함수는  사용자가 제시한 조건에 맞는 파일의 이름을 리스트 형식으로 반환한다.
 import os # os 모듈을 이용하여 운영체제가 지원하는 기능을 사용하여 디렉토리를 만들거나 옮기기 위한 기능을 제공한다.
 import sys # 파이썬 인터프리터가 제공하는 변수나 함수를 제어할 수 있는 방법을 제공한다.
def process(current_dir):
 percentage_test = 10;
 ‐----------------------
index_test = round(100/percentage_test) # 반올림한다.
for pathAndFilename in glob.iglob(os.path.join(current_dir, "*.jpg)) 실제로 동시에 모든 값을 저장하지 않고 glob과 동일한 값을 생성한다.
title.ext = os.path.splitext(os.path.basename(pathAndFilename) # 파일의 경로를 os.path.splitext 함수의 매개 변수로 넣어두면 두 개의 값이 반환되고 두 번째 값이 파일의 확장자
지은 — 어제 오후 7:07
IndexError: index 0 is out of bounds for axis 0 with size 0
Traceback (most recent call last):
  File "C:\Users\zziya\code1\tutorial\AI_Module.py", line 69, in <module>
    pred_x = pred_local[0]
IndexError: index 0 is out of bounds for axis 0 with size 0
지은 — 어제 오후 7:40
...
=================================================================
Total params: 13,832,772
Trainable params: 13,830,724
Non-trainable params: 2,048
_________________________________________________________________
접기
modelsummary_L_1651113386.txt
3KB
위에가 classification 아래가 localization
이미지
이미지
김채원 — 오늘 오후 2:34
정확도 그래프
김채원 — 오늘 오후 3:14
Localization.py 주석 미완성 
import os
...
# 신경망 층을 구성하기위해 Sequential()를 사용한다.
    ...
    #컨볼루션 레이어 (Conv2D)
# 필터로 입력 데이터의 특징 맵을 추출하고 한 칸 이동하며 연산한다.
Kenrel_size를 통해 맵의 크기를 조절한다. 
가로와 세로의 길이가 같으므로 정수 하나만 입력한다. 
패딩은 출력 데이터의 공간적 크기를 조절하기 위해 사용하며 Valid 혹은 same 값을 받음.
